...
/**
 * Builds iPhone-realism influencer photo prompts from a profile array,
 * ported from the React reference app's systemPrompt.js and Create.jsx.
 */
final class InfluencerPromptBuilder
{
    ...
    public static function buildThreeVariationPrompts(array $d, string $aspectRatio = '9:16', string $model = 'default'): array
    {
        ...
        if (self::isSoulModel($model)) {
            return [
                self::buildDirectPrompt($d, 'facing', ['model' => $model, 'forceProp' => $sessionProp()], $aspectRatio),
                self::buildDirectPrompt($d, 'angled', ['model' => $model, 'forceProp' => $sessionProp()], $aspectRatio),
                self::buildDirectPrompt($d, 'candid', ['forceOutdoor' => true, 'model' => $model, 'forceProp' => $sessionProp(true)], $aspectRatio),
            ];
        }
