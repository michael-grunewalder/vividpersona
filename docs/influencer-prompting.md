# Influencer prompting system

The influencer avatar prompts are built in PHP under `app/Support/Prompt/`. They
...
Every prompt (`InfluencerPromptBuilder::buildDirectPrompt`) has 10 sections:
...
| File | Contents |
| --- | --- |
| `InfluencerPromptBuilder.php` | `physicalDesc()`, `buildThreeVariationPrompts()`, `buildDirectPrompt()`, `selectWardrobe()`, `getStylingNote()`, `getBackstoryContext()` |
...
`app/Support/InfluencerOptions.php` (`VIBES`).
